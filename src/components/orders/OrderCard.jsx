...
export default function OrderCard({ order, index, onClick, canEdit }) {
  ...
  return (
    <Draggable draggableId={order.id} index={index} isDragDisabled={!canEdit}>
      {(provided, snapshot) => (
        <div
          ...
          className={cn(
            "bg-card rounded-xl p-4 border border-border shadow-sm transition-all duration-200 border-l-4",
            canEdit ? "cursor-grab hover:shadow-md active:cursor-grabbing" : "cursor-pointer hover:shadow-md",
