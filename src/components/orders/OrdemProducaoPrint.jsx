...
export default function OrdemProducaoPrint({ order, onClose }) {
  ...
  // Divide os itens em duas colunas (metade em cada)
  const half = Math.ceil(items.length / 2);
  const colA = items.slice(0, half);
  const colB = items.slice(half);

  // Preenche com linhas em branco até ROWS_PER_COL
  const padCol = (col) => {
    const padded = [...col];
    ...
    <div className="fixed inset-0 z-[100] bg-white overflow-y-auto">
      ...
      <div className="ordem-producao-print p-6 max-w-[280mm] mx-auto w-full">
        <div className="border-2 border-black p-5 text-black" style={{ fontFamily: 'Arial, Helvetica, sans-serif' }}>
          ...
          {/* Duas colunas de tabelas */}
          <div className="flex gap-4 mb-4">
            {renderColumn(colARows, 'A')}
            {renderColumn(colBRows, 'B')}
          </div>
