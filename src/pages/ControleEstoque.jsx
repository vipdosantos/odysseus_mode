...
export default function ControleEstoque() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {tab === 'insumos' ? `${supplies.length} insumos` : `${products.length} produtos`} cadastrados
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <div className="inline-flex rounded-lg bg-muted p-1">
            <button onClick={() => setTab('insumos')} className={cn("px-3 py-1.5 text-sm font-medium rounded-md transition", tab === 'insumos' ? "bg-background shadow text-foreground" : "text-muted-foreground")}>Insumos</button>
            <button onClick={() => setTab('produtos')} className={cn("px-3 py-1.5 text-sm font-medium rounded-md transition", tab === 'produtos' ? "bg-background shadow text-foreground" : "text-muted-foreground")}>Produtos (Treliças)</button>
          </div>
          {tab === 'insumos' && (
            <>
              ...
              <Button variant="outline" onClick={() => fileRef.current?.click()} disabled={importLoading}>
                ...
                {importLoading ? 'Lendo NF...' : 'Importar NF'}
              </Button>
      ...
      <div className={cn("space-y-6", tab !== 'insumos' && 'hidden')}>
      {/* Stats Insumos */}
      ...
      {/* Produtos (Treliças) */}
      <div className={cn("space-y-6", tab !== 'produtos' && 'hidden')}>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-card border rounded-2xl p-4">
            <p className="text-xs text-muted-foreground mb-1 flex items-center gap-1"><BarChart3 className="w-3.5 h-3.5" /> Total em Estoque</p>
